...
import type { Actor, DeclineBookingInput } from '@cerca/contract';
...
export class DeclineBookingUseCase {
  ...
  async execute(actor: Actor, bookingId: string, input: DeclineBookingInput): Promise<BookingRecord> {
    return this.tx.run(async () => {
      ...
      if (booking.status.kind !== 'requested') {
        throw new ConflictError({ code: 'INVALID_BOOKING_STATE', message: 'Only a requested booking can be declined.' });
      }
      return this.bookings.decline(bookingId, input.reason);
    });
  }
}
